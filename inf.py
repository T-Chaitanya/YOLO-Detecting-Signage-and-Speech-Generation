...
def plot_bboxes(image, boxes, labels=[], colors=[], score=True, conf=None):
    ...
    # if IN_COLAB:
    #     display(Image.fromarray(image))  # if used in Colab
    #     pass
    # else:
    #     display(Image.fromarray(image))  # if used in Python
    #     pass
...
path = "/Users/chaitanyatata/Downloads/test_onl/"
test_images = os.listdir("/Users/chaitanyatata/Downloads/test_onl/")


model = YOLO('/Users/chaitanyatata/PycharmProjects/Yolo_v8_Model_FP/runs/detect/train3/weights/best_m_75.pt')
system_name ="Mac"

for i in range(0, 3):
    image_path=path + test_images[random.randint(0, 10)]
    if test_images[i] ==".DS_Store" or ".DS_Store" in image_path:
        continue
    ...
    results = model.predict(image, imgsz=384)
    labels = ['accessibility', 'door', 'elevator sign', 'elevator', 'exit sign', 'fire alarm', 'fire extinguisher',
              'handle', 'left arrow', 'men-s washroom', 'person', 'push handle', 'right arrow', 'stair sign', 'trash can', 'water dispenser', 'women-s washroom']
    colors = []
    ann_image,label_names = plot_bboxes(image, results[0].boxes.data, labels, colors, score=True, conf=0.30)
    ...
    if system_name=="Mac":
        ...
        for j in label_names:
            speech = gTTS(text=j, lang='en', slow=False)
